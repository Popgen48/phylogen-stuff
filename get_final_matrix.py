# Usage: python3 get_final_matrix.py <vcf_file_path> <threshold> <group_size> <spa_criterion>

import pandas as pd
import math
import sys
import get_sp_alleles2

vcf_path = sys.argv[1]
threshold = float(sys.argv[2]) 
group_size = int(sys.argv[3])
spa_criterion = int(sys.argv[4])
...
# Create DataFrame
df = pd.DataFrame(data, columns=['population_id', 'animal_id_1', 'animal_id_2', 'UAR score'])
print(df)
...
def calculate_genetic_distance(row):
    ...
    pop_i = row['population_id']

    interacted_within = df[(df['animal_id_2'] == animal_i) & (df['population_id'] == pop_i)]
    ...
    if pop_i not in mean_within:
        population_breeds = df[(df['population_id'] == pop_i)]
...
def calculate_max_uar(row):
    animal_i = row['animal_id_1']
    population_i = row['population_id']
    
    # Filter rows where animal_id_1 is different from current animal_i and from a different population
    foreign_breeds = df[(df['animal_id_1'] != animal_i) & (df['population_id'] != population_i)]
    ...
    breed_avg_uar = foreign_breeds.groupby('population_id')['UAR score'].mean()
...
# Get the spa counts
spa_counts = get_sp_alleles2.get_spa(vcf_path, threshold, group_size, spa_criterion)
...
for animal_i in unique_animals:
    ...
    spa = spa_counts[spa_counts['Animal_ID'] == animal_i]['spa'].values[0]
    animal_data = {
        'Population_ID': pop_dict[animal_i],
        'Animal_ID': animal_i,
        'Genetic Distance': gd,  
...
result_df['Population_ID'] = pd.to_numeric(result_df['Population_ID'])
result_df = result_df.sort_values(by=['Population_ID'])
result_df.reset_index(drop=True, inplace=True)
result_df.to_csv('final_result.txt', sep='\t\t', index=False)
print('-----------------Final Result--------------------')
print(result_df)